...
//       setTimeout(typeEffect, 0); // No delay when hero-text is blank
//     } else if (letterIndex === 0) {
//       textElement.textContent = currentWord.slice(0, letterIndex);
//       direction = 1;
//       wordIndex = (wordIndex + 1) % words.length;
//       setTimeout(typeEffect, 4000); // Delay before next word typing
//     } else {
//       textElement.textContent = currentWord.slice(0, letterIndex);
//       letterIndex--;
...
function typeEffect() {
  ...
  if (direction === 1) {
    ...
    if (letterIndex === 0) {
      ...
      direction = 1;
      wordIndex = (wordIndex + 1) % words.length;
      setTimeout(typeEffect, currentWord === '' ? 0 : 4000); // No delay when hero-text is empty
    } else {
      textElement.textContent = currentWord.slice(0, letterIndex);
      letterIndex--;
      setTimeout(typeEffect, 100); // Delay between deleting each letter
